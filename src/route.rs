...
pub fn handle_blog_list_page(_: &mut Request) -> IronResult<Response> {
    ...
    if posts.is_empty() {
        ...
        handle_with_template2("blog_list_noposts", template_data)
    } else {
        let mut template_data = BTreeMap::new();
        template_data.insert(String::from("posts"), posts);
        handle_with_template2("blog_list", template_data)
    }
}
...
pub fn handle_blog_post_page(request: &mut Request) -> IronResult<Response> {
    ...
    handle_with_template2("blog_post", template_data)
}
...
fn handle_with_template2<T: ::serde::Serialize>(name: &str, data: T) -> IronResult<Response> {
    Ok(Response::with(
        (status::Ok, Template::new(name, data))
    ))
}
